...
use bevy_voxel_mesh::{
    extract_greedy_lod_buffer, visible_greedy_quads, ChunkShape, GreedyVisitedBuffer, MergeVoxel,
    MeshVoxel, PopBuffer, QuadsBuffer, VoxelVisibility,
};
...
fn generate_visible_faces_mesh(voxels: &[Voxel]) -> Mesh {
    let mut greedy_buffer = PopBuffer::new();
    let mut visited_buffer = GreedyVisitedBuffer::new(voxels.len());
    let mut buffer = QuadsBuffer::new();

    visible_greedy_quads::<34, 34, 34, 4, _>(voxels, &mut greedy_buffer, &mut visited_buffer);
    let lod = 1;
    extract_greedy_lod_buffer(&greedy_buffer, &mut buffer, lod);

    println!("{:?}", buffer.num_quads());
